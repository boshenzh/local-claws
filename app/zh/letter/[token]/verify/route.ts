...
export async function POST(
  ...
  if (!passcode) {
    return new Response("需要提供口令", { status: 400 });
  }
    ...
    const html = retroDocument(
      ...
      `<main class="letter-shell">
        <section class="letter">
          ...
          <p class="kicker">LocalClaws // 邀请终端</p>
          <h1 class="error-title">访问被拒绝</h1>
          <p class="error-copy">${escapeHtml(result.message)}</p>
          <div class="actions">
            <a class="btn" href="/zh/letter/${escapeHtml(token)}">重新输入口令</a>
            <a class="btn" href="/zh">返回 LocalClaws</a>
          </div>
        </section>
      </main>`
    );
  ...
  const privateInviteImageSection =
    details.privateInviteImageUrl && details.privateInviteImageUrl.trim()
      ? `<article class="private-image-card">
          <h2>主办方私密附件</h2>
          <p class="meta-label">仅对已确认参与者可见。</p>
          <div class="private-image-frame">
            <img
              class="private-image-preview"
              src="${escapeHtml(details.privateInviteImageUrl)}"
              alt="${escapeHtml(details.privateInviteImageCaption || "聚会私密附件")}"
              loading="lazy"
            />
          </div>
          <p class="meta-label" style="margin-top:10px;">
            ${escapeHtml(details.privateInviteImageCaption || "主办方分享的私密图片")}
          </p>
          <p class="meta-label">若预览失败，请直接打开图片。</p>
          <div class="private-image-link">
            <a class="btn" href="${escapeHtml(details.privateInviteImageUrl)}" target="_blank" rel="noopener noreferrer">打开私密图片</a>
          </div>
        </article>`
      : "";
    ...
    `<main class="letter-shell">
      <article class="letter" id="invite-letter-capture">
        ...
        <div class="party-chips">
          <span class="party-chip mint">已确认来宾</span>
          <span class="party-chip rose">准备就绪</span>
        </div>
        <p class="kicker">LocalClaws 官方聚会邀请函</p>
        <h1>${escapeHtml(details.meetupName)}</h1>
        <p class="subtitle">你的入场口令已验证。</p>

        <div class="rule"></div>

        <section>
          <section class="meta-grid" aria-label="邀请详情">
            <div class="meta-card">
              <p class="meta-label">集合时间</p>
              <p class="meta-value">${escapeHtml(localTimeText)}</p>
            </div>
            <div class="meta-card">
              <p class="meta-label">集合地点</p>
          ...
          <h2>主办方备注</h2>
          <div class="notes">${escapeHtml(details.hostNotes || "主办方暂无额外备注。")}</div>
        ...
        <section class="utility-grid">
          <article class="radar-card">
            ...
            <p class="meta-label" style="margin-top:10px;">地点说明</p>
            <div class="location-description">${escapeHtml(details.exactLocation)}</div>
            <p class="kicker" style="margin-top:10px;">精确地点仅对已确认参与者解锁。</p>
          </article>
        </section>

        <div class="actions" aria-label="Party pack actions">
          <button class="btn" id="save-calendar-btn" type="button">保存到日历 (.ics)</button>
          <button class="btn" id="save-image-btn" type="button">保存为图片 (PNG)</button>
          ${
            details.exactLocationLink
              ? `<a class="btn" href="${escapeHtml(details.exactLocationLink)}" target="_blank" rel="noopener noreferrer">打开地图</a>`
              : ""
          }
          <a class="btn" href="/zh">返回看板</a>
        </div>
      ...
      (() => {
        ...
        const replaceMapIframesForCapture = (node) => {
          node.querySelectorAll('iframe').forEach((frame) => {
            ...
            replacement.textContent = '浏览器中可查看地图预览。实时导航请使用“打开地图”。';
